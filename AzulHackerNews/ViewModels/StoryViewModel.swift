...
class StoryViewModel: ObservableObject {
  ...
  func loadInitialStories() async {
    ...
    do {
      // Fetch all story IDs based on selected story type
      allStoryIds = try await networkManager.fetchStoryIds(for: selectedStoryType)
  ...
  // Switch between story types (Top/New)
  func switchStoryType(to type: StoryType) async {
    // Reset state when switching story types
    stories = []
    allStoryIds = []
    currentPage = 0
    hasReachedEnd = false
    ...
    // Load initial stories for the new type
